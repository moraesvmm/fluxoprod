...
## VISTORIA 41 - Log�stica NFe (Build + Instrumenta�?o)
- **Data:** 29/04/2026
- **Status:** PENDENTE (Realizar vistoria o mais r�pido poss�vel)
- **Altera�?es:**
  - Corre�?es de build TypeScript/JSX (Cat�logo, Configura�?es, Vendas).
  - Ajuste de compatibilidade `xml-crypto` (assinatura XML) para a vers?o instalada.
  - Instrumenta�?o server-side na rota `/api/fiscal/nfe/emitir` com fallback de log NDJSON local para auditoria runtime.

---

## VISTORIA 40 - Intelig�ncia Financeira & CMV
- **Data:** 29/04/2026
- **Status:** CONCLU�DO
- **Altera��es:**
  - Implementa��o de integra��o autom�tica Venda -> Financeiro.
  - Motor de DRE Real (Demonstrativo de Resultados) com c�lculo de CMV.
  - Sistema de Concilia��o Banc�ria real via parser de arquivos OFX.
  - KPI de Patrim�nio em Estoque (Capital Imobilizado) no Dashboard.
  - Atualiza��o dos cards de checkout com as novas features.
...
## VISTORIA 39 (PENDENTE): Implementa��o Nativa do Motor de NFe (Custo Zero) - 29/04/2026

### Objetivo
Implementar emiss�o nativa de NFe 4.00 sem depend�ncia de SaaS (FocusNFe/NFe.io).
...
- **api.ts [MODIFICADO]**: Expans�o de interfaces.
- **rpc_fiscal_expansion.sql [NOVO]**: Expans�o SQL.
