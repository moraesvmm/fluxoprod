-- ==========================================
-- CRIAR RPCs FALTANTES NO SCHEMA PUBLIC
-- ==========================================
-- Execute este SQL no painel do Supabase (SQL Editor)
-- Objetivo: Criar RPCs de criação que estão faltando no schema public
-- Permissão: Use a service_role para executar este SQL

-- 1. RPC para criar cliente
DROP FUNCTION IF EXISTS public.tenant_criar_cliente(VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR);
...
-- 2. RPC para criar produto
DROP FUNCTION IF EXISTS public.tenant_criar_produto(VARCHAR, TEXT, VARCHAR, NUMERIC, VARCHAR, NUMERIC, VARCHAR);
...
-- 3. RPC para processar venda (transacional)
DROP FUNCTION IF EXISTS public.tenant_processar_venda(UUID, VARCHAR, UUID, VARCHAR, NUMERIC, VARCHAR, JSONB);
...
-- 4. RPC para excluir cliente
DROP FUNCTION IF EXISTS public.tenant_excluir_cliente(UUID);
...
-- 5. RPC para excluir produto
DROP FUNCTION IF EXISTS public.tenant_excluir_produto(UUID);
...
-- 6. RPC para excluir OS
DROP FUNCTION IF EXISTS public.tenant_excluir_os(UUID);
...
-- 7. RPC para excluir obra
DROP FUNCTION IF EXISTS public.tenant_excluir_obra(UUID);
