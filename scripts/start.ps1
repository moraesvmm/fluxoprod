...
# Configurar Node usando diretório local
...
if ($nodeDir) {
  ...
  Write-Host "Node configurado via diretório local"
} else {
  Write-Host "AVISO: Node portátil não encontrado."
}
...
# Frontend
if ($nodeExe) {
  Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd `"$repoRoot\apps\web`"; `"$nodeExe`" npx next dev -- --port 3000"
  Write-Host "Frontend Next.js em http://localhost:3000"
} else {
  Write-Host "AVISO: Node não encontrado. Frontend não iniciado."
}
