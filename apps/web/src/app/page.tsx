...
import {
  ...
  Play, ChevronRight, Cpu, Lock, Gauge,
} from "lucide-react";
...
/* ─── DATA ─── */
const features = [
  { icon: BarChart3, title: "Dashboard Executivo", description: "Visão 360° do seu negócio com gráficos inteligentes, KPIs em tempo real e insights acionáveis que transformam dados em decisões." },
  { icon: Package, title: "Gestão de Estoque", description: "Controle total do inventário com alertas de estoque mínimo, rastreabilidade de lote e integração automática com vendas." },
  { icon: Users, title: "CRM Integrado", description: "Pipeline visual de vendas, histórico completo de interações e automações que convertem leads em clientes fiéis." },
  { icon: TrendingUp, title: "Financeiro Completo", description: "Contas a pagar e receber, fluxo de caixa projetado, conciliação bancária e DRE automático com precisão fiscal." },
  { icon: ShieldCheck, title: "Multi-Tenant Seguro", description: "Arquitetura isolada por empresa com RLS nativo. Seus dados jamais se misturam — segurança de nível enterprise." },
  { icon: Zap, title: "Módulos Sob Demanda", description: "Ative apenas o que precisa. Cada módulo é independente e escalável, do financeiro ao ponto de venda." },
];
...
export default function LandingPageV2() {
  ...
  return (
    <div className={`relative min-h-screen overflow-x-hidden transition-colors duration-500 ${d ? "bg-[#060611] text-slate-100" : "bg-[#fafafe] text-slate-900"}`}>
        ...
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20">
            <Link href="/" className="flex items-center gap-3 group">
              <Image src="/logo-fluxo.png" alt="Fluxo" width={38} height={38} className="object-contain drop-shadow-[0_0_14px_rgba(139,92,246,0.5)] transition-transform group-hover:scale-110" />
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <button onClick={toggleDark} className={`p-2 rounded-xl transition-colors ${d ? "text-slate-400 hover:text-white hover:bg-white/5" : "text-slate-500 hover:text-slate-900 hover:bg-slate-100"}`} aria-label="Toggle Dark Mode">
            ...
            <div className="flex items-center gap-2 md:hidden">
              <button onClick={toggleDark} className={`p-2 rounded-xl ${d ? "text-slate-400" : "text-slate-500"}`} aria-label="Dark Mode">
                {d ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
              </button>
        ...
        <motion.div style={{ opacity: heroOpacity, scale: heroScale }} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} className={`mt-7 text-lg sm:text-xl max-w-2xl mx-auto leading-relaxed ${d ? "text-slate-400" : "text-slate-600"}`}>
              <strong className={d ? "text-white" : "text-slate-800"}>Fluxo</strong> centraliza finanças, estoque, CRM e vendas em uma única plataforma segura — para que você foque no que realmente importa: <GradientText className="font-semibold">crescer.</GradientText>
            </motion.p>
            ...
            {/* Trust row */}
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.5 }} className={`mt-14 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-xs font-medium ${d ? "text-slate-500" : "text-slate-400"}`}>
              {[
      ...
      <section id="funcionalidades" className="py-24 sm:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection className="text-center mb-16 sm:mb-20">
            <SectionBadge icon={Layers} label="Módulos" dark={d} />
            ...
            <p className={`mt-4 text-lg max-w-2xl mx-auto ${d ? "text-slate-400" : "text-slate-500"}`}>Módulos independentes que trabalham juntos. Ative sob demanda, escale sem limites.</p>
          </AnimatedSection>
      ...
      {/* ═══ BENEFITS ═══ */}
      <section id="vantagens" className={`py-24 sm:py-32 relative ${d ? "" : ""}`}>
        <div className="absolute inset-0 -z-10">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <AnimatedSection>
              <SectionBadge icon={Star} label="Vantagens" dark={d} />
              <h2 className={`mt-5 text-3xl sm:text-4xl font-extrabold tracking-tight mb-6 ${d ? "text-white" : "text-slate-900"}`}>
                Por que empresas escolhem o <GradientText>Fluxo?</GradientText>
              </h2>
              <p className={`text-lg mb-8 leading-relaxed ${d ? "text-slate-400" : "text-slate-500"}`}>
                Desenvolvido com tecnologia de ponta para negócios que levam gestão a sério. Sem complexidade desnecessária, sem custos ocultos.
              </p>
              <ul className="space-y-4">
                {benefits.map((b, i) => (
                  <motion.li key={b} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }} className={`flex items-center gap-3 ${d ? "text-slate-300" : "text-slate-700"}`}>
            ...
            <AnimatedSection delay={0.15}>
              <motion.div whileHover={{ scale: 1.02 }} transition={{ duration: 0.4 }} className="relative p-8 sm:p-12 rounded-3xl bg-gradient-to-br from-violet-600 via-purple-600 to-indigo-700 text-white shadow-2xl shadow-violet-500/25 overflow-hidden">
                ...
                <div className="relative">
                  ...
                  <h3 className="text-2xl sm:text-3xl font-extrabold mb-4 leading-tight">Infraestrutura que escala com você.</h3>
                  <p className="text-violet-100 text-sm sm:text-base leading-relaxed mb-8">Banco de dados por empresa, cache inteligente, APIs RESTful e deploy contínuo. Tudo em tempo real, sem downtime.</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <Image src="/logo-fluxo.png" alt="Fluxo" width={28} height={28} className="object-contain drop-shadow-[0_0_8px_rgba(139,92,246,0.4)]" />
              <span className="text-sm font-semibold bg-gradient-to-r from-violet-500 to-indigo-400 bg-clip-text text-transparent">Fluxo</span>
            </div>
