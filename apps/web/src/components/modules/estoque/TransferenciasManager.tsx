...
export default function TransferenciasManager() {
  ...
  const handleSalvarLocal = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toastError("Erro ao criar local: " + (err.message || "Tente novamente."));
    }
  };
  ...
  const handleDesativarLocal = async () => {
    ...
    try {
      ...
      toastError("Erro ao desativar local: " + (err.message || "Tente novamente."));
    } finally {
  ...
  const handleCriarTransferencia = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toastError("Erro ao criar transferência: " + (err.message || "Tente novamente."));
    }
  };
  ...
  const handleConcluirTransferencia = async () => {
    ...
    try {
      ...
      toastError("Erro ao concluir transferência: " + (err.message || "Tente novamente."));
    } finally {
  ...
  const handleCancelarTransferencia = async () => {
    ...
    try {
      ...
      toastError("Erro ao cancelar transferência: " + (err.message || "Tente novamente."));
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {loadingTransferencias ? (
          <div className="text-center py-8 text-slate-500">Carregando transferências...</div>
      ...
      <ConfirmModal
        ...
        message="Tem certeza que deseja cancelar esta transferência? O estoque será devolvido à origem."
        variant="danger"
      />
    </div>
  );
